...
void M2006_getInfo(uint8_t *CanRxBuff, uint32_t MotorID)
{
	...
	M2006s[StdId].realAngle = (uint16_t)(CanRxBuff[0]<<8 | CanRxBuff[1]);
	M2006s[StdId].realSpeed = (int16_t)(CanRxBuff[2]<<8  | CanRxBuff[3]);
	M2006s[StdId].realTorque = (int16_t)(CanRxBuff[4]<<8 | CanRxBuff[5]);
